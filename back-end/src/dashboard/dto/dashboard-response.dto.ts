...
import { OotdItemDto, OotdResponseDto } from '../../recommendation/dto/ootd-response.dto';
...
export class WardrobeStatsDto {
  @ApiProperty({ example: 42, description: 'Total number of items in the wardrobe' })
  totalItems: number;

  @ApiProperty({ example: 1240.5, description: 'Total estimated value of all items (sum of prices)' })
  totalValue: number;
}
...
export class DashboardResponseDto {
  @ApiProperty({ type: WeatherResponseDto, description: 'Current weather at the user\'s location' })
  weather: WeatherResponseDto;

  @ApiProperty({ type: OotdResponseDto, description: 'AI-powered Outfit Of The Day recommendation' })
  ootd: OotdResponseDto;

  @ApiProperty({ type: [RecentItemDto], description: 'Last 5 items added to the wardrobe' })
  recentItems: RecentItemDto[];

  @ApiProperty({ type: WardrobeStatsDto, description: 'Aggregate statistics of the wardrobe' })
