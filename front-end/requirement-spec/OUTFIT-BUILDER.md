...
- **Step 2: DTOs**
    - Create `CreateOutfitDto` and `UpdateOutfitDto` with comprehensive `class-validator` decorators.
- **Step 3: Service & Controller**
    ...
    - Write `OutfitController` to expose the corresponding RESTful APIs.
...
- **Step 1: Types/Interfaces**
    - Define `IOutfit` and `CreateOutfitPayload` interfaces synchronized with the Backend.
- **Step 2: API Integration**
    - Update the API service file (using Axios) to add functions for creating and fetching Outfits.
- **Step 3: UI Components & UX Rules**
    ...
    - **UI/UX Consistency:** Ensure the new UI seamlessly integrates with the application's existing design system. Reuse existing common components and strictly follow the current styling architecture (Tailwind CSS/styled-components).
